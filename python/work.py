...
class CosineSimilarity(object):
    ...
    def main(self):
        # 去除停用词
        jieba.analyse.set_stop_words(r'G:\test\stopwords.txt')
...
if __name__ == '__main__':
    with open(r'G:\test\orig.txt', 'r',encoding='utf-8') as x, open(r'G:\test\orig_0.8_del.txt', 'r',encoding='utf-8') as y:
        content_x = x.read()
        content_y = y.read()
        similarity = CosineSimilarity(content_x, content_y)
        similarity = similarity.main()
        print('相似度: %.1f%%' % (similarity*100))
        f3 = open(r'G:\test\result.txt','w')
